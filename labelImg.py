...
from libs.json_io import JSON_EXT,JSONReader
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        # Load predefined classes to the list 加载预定义的类
        ...
        save_format = action('Json', self.change_format,
                      'Ctrl+', 'json', getStr('changeSaveFormat'), enabled=True)
    ...
    def set_format(self, save_format):
        if save_format == FORMAT_PASCALVOC:
            ...
            self.actions.save_format.setIcon(newIcon("json"))
            self.usingPascalVocFormat = False
            self.usingYoloFormat = False
            self.usingJSONFormat=True
    ...
    def editLabel(self):
        ...
        if not item:
            return
        label,group = self.labelDialog.popUp(item.label,item.group)
        if label is not None:
            item.group=group
            item.setText(label+",g:"+group)
    ...
    def addLabel(self, shape):
        ...
        item = HashableQListWidgetItem(shape.label+",g:"+shape.group)
        item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
        item.setCheckState(Qt.Checked)
        item.setBackground(generateColorByText(shape.label))
        item.group=shape.group
    ...
    def loadLabels(self, shapes):
        s = []
        for label, points, line_color, fill_color, difficult,group in shapes:
    ...
    def saveLabels(self, annotationFilePath):
        ...
        def format_shape(s):
            return dict(label=s.label,
                       ...
                       # add chris
                        difficult = s.difficult,group=s.group)
    ...
    def newShape(self):
        ...
        if not self.useDefaultLabelCheckbox.isChecked() or not self.defaultLabelTextLine.text():
            ...
            if self.singleClassMode.isChecked() and self.lastLabel:
                ...
                text,group = self.labelDialog.popUp(text=self.prevLabelText)
                self.lastLabel = text
        else:
            text,group = self.defaultLabelTextLine.text(),"-1"
        ...
        if text is not None:
            ...
            shape = self.canvas.setLastLabel(text,group, generate_color, generate_color)
            self.addLabel(shape)
    ...
    def loadJSONByFilename(self, jsonPath):
        ...
        tJSONReader = JSONReader(jsonPath)
        shapes = tJSONReader.getShapes()
        self.loadLabels(shapes)
        self.canvas.verified = tJSONReader.verified
